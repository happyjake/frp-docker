...
RUN wget https://github.com/fatedier/frp/releases/download/v${VERSION}/frp_${VERSION}_darwin_arm64.tar.gz
RUN tar -xzf frp_${VERSION}_darwin_arm64.tar.gz

# Stage 2: Copy frps binary to a new image
FROM alpine
COPY --from=builder /frp_${VERSION}_darwin_arm64/frps /usr/local/bin/frps
